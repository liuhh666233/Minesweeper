...
### 存储内容
- 游戏记录：包含游戏时间、难度级别、胜负结果等
- 玩家统计：不同难度级别的胜率、平均用时等
- 排行榜：按难度级别区分的最佳成绩

### 数据文件
- 默认数据库文件位于用户主目录下的 `~/.minesweeper/minesweeper.db`
- 可以通过环境变量 `MINESWEEPER_DB_PATH` 自定义数据库文件位置
- 数据库文件和目录会自动创建，无需手动配置
